...
def save_state():
    with open('2_visualization/analysis_results_' + ''.join(map(str,filters)) + '.csv', 'wb') as out:
...
def save_core(filters):
    with open('2_visualization/analysis_core' + ''.join(map(str,filters)) + '.csv', 'wb') as out:
        writer = csv.writer(out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        printlist = []
        for con in concepts:
...
def save_core_bars(filters):
    with open('2_visualization/analysis_core_bars' + ''.join(map(str,filters)) + '.csv', 'wb') as out:
        for con in concepts:
            writer = csv.writer(out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
def save_adds(filters):
    with open('2_visualization/analysis_adds' + ''.join(map(str,filters)) + '.csv', 'wb') as out:
        for con in concepts:
            writer = csv.writer(out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            printlist = {t:[] for t in con.fixIntervals}
            add_no_key = []
...
def save_rems(filters):
    with open('2_visualization/analysis_rems' + ''.join(map(str,filters)) + '.csv', 'wb') as out:
        for con in concepts:
            writer =  csv.writer(out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            printlist = {t:[] for t in con.fixIntervals}
            rem_no_key = []
